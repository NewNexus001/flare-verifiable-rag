...
import {IKmsVerifiedWallet} from "./interfaces/IKmsVerifiedWallet.sol";
import {IZkTlsRelayer} from "./interfaces/IZkTlsRelayer.sol";
 ...
 */
contract VerifiableRAG is Ownable, ReentrancyGuard, IKmsVerifiedWallet {
    ...
    /// The zkTLS relayer contract (ZkTlsRelayer.sol) the enclave posts
    /// sub-second Web2 attestations to (owner-configurable via
    /// {updateZkTlsRelayer}, Prompt 272). address(0) = unset; reads revert
    /// `ZeroZkTlsRelayer` until set (fail-closed).
    IZkTlsRelayer public zkTlsRelayer;
    ...
    /// Registered enclave MPC wallet addresses (Phase 13, P250). The composed
    /// public key of the 2-of-2 KMS key — see enclave/enclave_grpc/src/kms.
    mapping(address signer => bool registered) public kmsSigners;
    ...
    /// Reverts when submitAttestation receives empty attestation data (a
    /// zero-length payload would be silently unverifiable — fail-closed).
    error EmptyAttestationData();

    // ---- Phase 14: zkTLS relayer (Prompt 272) -----------------------------

    /// Reverts when the zkTLS relayer address is the zero address.
    error ZeroZkTlsRelayer();

    /// Reverts when the new relayer address equals the current one (no-op).
    error UnchangedZkTlsRelayer();

    // ---- Phase 13: KMS MPC wallet (Prompts 249-250) -----------------------

    /// Reverts when the owner registers the zero address (KMS gate).
    error ZeroKmsSigner();

    /// Reverts when an address is already registered (no-op).
    error KmsSignerAlreadyRegistered(address signer);

    /// Reverts when an address is not registered (no-op).
    error KmsSignerNotRegistered(address signer);

    /// Emitted when the enclave executes a KMS-signed action on-chain
    /// (the KMS gate errors/events UnauthorizedSigner, ZeroSigner,
    /// KmsSignerRegistered, KmsSignerDeregistered are declared in
    /// IKmsVerifiedWallet).
    event KmsActionExecuted(
    ...
    /// Emitted when the owner (re)configures the zkTLS relayer address.
    event ZkTlsRelayerUpdated(
    ...
    /// @notice Set the zkTLS relayer contract the enclave posts sub-second
    ///         Web2 attestations to (owner only, Prompt 272). The relayer is
    ///         a PROJECT contract (ZkTlsRelayer.sol) — it is NOT a Flare
    ///         periphery contract, so unlike FdcHub/FdcVerification/FtsoV2 it
    ///         is not resolvable from the FlareContractRegistry. It is
    ///         therefore held as an owner-managed reference (the same pattern
    ///         as {priceFeedId}), resolving to a live address on Flare.
    /// @dev Reverts `ZeroZkTlsRelayer` on address(0) and
    ///      `UnchangedZkTlsRelayer` on a no-op.
    /// @dev Emits {ZkTlsRelayerUpdated}.
    function updateZkTlsRelayer(address _newRelayer) external onlyOwner {
        if (_newRelayer == address(0)) {
            revert ZeroZkTlsRelayer();
        }
        if (_newRelayer == address(zkTlsRelayer)) {
            revert UnchangedZkTlsRelayer();
        }
        emit ZkTlsRelayerUpdated(address(zkTlsRelayer), _newRelayer);
        zkTlsRelayer = IZkTlsRelayer(_newRelayer);
    }

    /// @notice Read the verified Web2 data recorded by the zkTLS relayer for
    ///         a URL hash (Prompt 272) — the sub-second attestation path's
    ///         on-chain result. Reverts `ZeroZkTlsRelayer` until a relayer is
    ///         configured (fail-closed: never answers from a zero address).
    function verifiedZkTlsData(
        bytes32 _urlHash
    ) public view returns (bytes32 dataHash, uint256 verifiedAt) {
        if (address(zkTlsRelayer) == address(0)) {
            revert ZeroZkTlsRelayer();
        }
        IZkTlsRelayer.VerifiedData memory v = zkTlsRelayer.verifiedWeb2Data(_urlHash);
        return (v.dataHash, v.verifiedAt);
    }
    ...
    /// @notice Register an enclave MPC wallet address (owner only). The
    ///         address is the Ethereum address of the COMPOSED public key
    ///         (Q = S_enclave·G + S_client·G) — computable off-chain without
    ///         ever reconstructing the key.
    /// @dev Reverts `ZeroKmsSigner` on address(0) and
    ///      `KmsSignerAlreadyRegistered` on a no-op.
    /// @dev Emits {KmsSignerRegistered}.
    function registerKmsSigner(address _signer) external onlyOwner {
        if (_signer == address(0)) {
            revert ZeroKmsSigner();
        }
        if (kmsSigners[_signer]) {
            revert KmsSignerAlreadyRegistered(_signer);
        }
        kmsSigners[_signer] = true;
        emit KmsSignerRegistered(_signer);
    }

    /// @notice Deregister an enclave MPC wallet address (owner only).
    /// @dev Reverts `KmsSignerNotRegistered` on a no-op.
    /// @dev Emits {KmsSignerDeregistered}.
    function deregisterKmsSigner(address _signer) external onlyOwner {
        if (!kmsSigners[_signer]) {
            revert KmsSignerNotRegistered(_signer);
        }
        kmsSigners[_signer] = false;
        emit KmsSignerDeregistered(_signer);
    }

    /// @notice True when `_signer` is a registered enclave MPC wallet address
    ///         (IKmsVerifiedWallet).
    function isKmsSigner(address _signer) external view override returns (bool) {
        return kmsSigners[_signer];
    }

    /// @notice Recover the signer of `_hash` from `(v, r, s)` and require it
    ///         to be a registered enclave MPC wallet address (the ecrecover
    ///         gate — identical math to the enclave's k256 signatures).
    /// @return signer The recovered, registered signer address.
    ...
    function requireKmsSignature(
        ...
        if (!kmsSigners[signer]) {
            revert UnauthorizedSigner(signer);
        }
    }

    /// @notice Execute an enclave-initiated action authenticated by a KMS MPC
    ///         signature (Prompt 250). The enclave signs the action hash with
    ///         its 2-of-2 MPC key; any caller can relay it on-chain; the
    ///         contract verifies the recovered signer is registered.
    ///
    /// The action itself is a recorded intent (actionId = keccak of a
    /// domain-separated action payload) — the same pattern as the attestation
    /// commitment, so downstream consumers can prove the enclave authorized
    /// it. The domain separator prevents cross-context replay between
    /// different action payloads.
    ///
    /// @param _actionHash keccak256 of the action payload the enclave signed
    ///                    (e.g. keccak256(abi.encode("settle", queryHash,
    ///                    settlementQuantity))).
    /// @param _v          ECDSA recovery id (0 or 1 for EIP-1559 signatures).
    /// @param _r          ECDSA r.
    /// @param _s          ECDSA s (must be low-s — enforced by the enclave
    ///                    signer; the contract accepts what ecrecover returns).
    /// @return signer    The verified enclave MPC wallet address.
    /// @dev Reverts `ZeroSigner` / `UnauthorizedSigner` (see
    ///      {requireKmsSignature}).
    /// @dev Emits {KmsActionExecuted}.
    function executeKmsSignedAction(
        bytes32 _actionHash,
        uint8 _v,
        bytes32 _r,
        bytes32 _s
        ...
        signer = requireKmsSignature(_actionHash, _v, _r, _s);
        emit KmsActionExecuted(signer, _actionHash, block.timestamp);
    }
    ...
    function submitAttestation(
        ...
        if (_attestationData.length == 0) {
            revert EmptyAttestationData();
        }
        ...
        // _attestationData is length-validated above; its content is
        // forwarded to FdcHub verification in later prompts. Keeping it a
        // named parameter pins the canonical ABI shape.
    }
