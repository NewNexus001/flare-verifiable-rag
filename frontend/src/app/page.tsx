...
function LivePrice() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // keep the verified default (6dp) — surface the failure for debugging
        console.warn("FtsoV2 decimals read failed, keeping 6dp default", err);
      }
    })();
  }, [contractAddress]);
  ...
  if (isError || data === undefined) {
    return <p style={{ color: "#ff9d9d" }}>Live read failed (RPC or contract).</p>;
  }
  ...
  return (
    ...
    >
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        >
          <div>
            <h1 style={{ margin: "0 0 0.35rem", fontSize: "2rem", letterSpacing: "-0.02em" }}>
