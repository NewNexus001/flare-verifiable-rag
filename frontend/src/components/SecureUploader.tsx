...
export function SecureUploader({
  ...
  const onFile = useCallback(async (file: File | undefined) => {
    ...
    if (file.size > MAX_BYTES) {
      ...
      reportDiagnostic("warn", "SecureUploader", "file rejected: exceeds demo cap", `${file.size} bytes`);
      return;
    }
    setBusy(true);
    try {
      ...
      const msg = (e as Error).message ?? "Encryption failed.";
      setError("Encryption failed in the browser. Please retry.");
      reportDiagnostic("error", "SecureUploader", "client-side encryption failed", msg);
    } finally {
      setBusy(false);
    ...
    try {
      ...
      if (!res.ok) {
        reportDiagnostic(
          "error",
          "SecureUploader",
          `blind-proxy query returned HTTP ${res.status}`,
      ...
      const msg = (e as Error).message ?? "Proxy submission failed.";
      setError("Submission failed. Please retry.");
      reportDiagnostic("error", "SecureUploader", "blind-proxy submission failed", msg);
      onExecutionRecord?.({
        status: "error",
        detail: msg,
        timestamp: new Date().toISOString(),
